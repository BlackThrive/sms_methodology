---
title: "Black Thrive Shared Measurement System Methodology and Results"
date: "November 2021"
output:
  word_document:
...
This indicator describes the number of pupils achieving at least Level 4 in GCSE Maths and English. Data were acquired from *Key Stage 4 performance 2019/20*, produced by the Department for Education. The variables of interest were total number of pupils (*t_pupils*), total number achieving at least Level 4 (*t_l2basics_94*), and percentage achieving at least Level 4 (*pt_l2basics_94*). Subsets of this data were produced for Haringey and Birmingham by filtering the data according to the following criteria:
...
This indicator describes the number of children looked after by the local authority. Data were acquired from *Children looked after in England including adoption: 2018 to 2019*, produced by the Department for Education. The following filter was applied to acquire data specific to Haringey/Birmingham:
...
The procedure for Stop and Search data differed by local authority. For Haringey, Stop and Search data were acquired via the Metropolitan Police Stop and Search Dashboard data (published by the Metropolitan Police Service). Data for just Haringey was acquired by filtering the dataset by the variable *Borough of Stop*. For the purposes of the present analysis, an individual's ethnicity was taken as the ethnicity attributed to the individual by the officer (variable name *EA Group*). The time period this data covered was the 12 months to the end of August 2021.

For Birmingham, the Police API (published by the Single Online Home National Digital Team) was used to query Stop and Search data. To do this, it was first necessary to define the area of Birmingham using latitude and longitude coordinates. Coordinates of the city boundary were acquired from https://mapit.mysociety.org/area/2514.html. This provided 3634 coordinate points that define the area of Birmingham, as below: 
...
It was not appropriate to use officer-defined ethnicity for the Birmingham dataset because for `r round(appearance_freq_data$percentage[which(appearance_freq_data$officer_defined_ethnicity == "NULL")],2)`% of records this data was not reported (see the tables below). In contrast, self-defined ethnicity (*self_defined_ethnicity*) was reported for `r round(self_perc_recorded,2)`% of records, so this variable was used as the ethnicity variable in the present analysis.

```{r include = TRUE, echo = FALSE}
appearance_freq_data %>% kable(caption = "Officer-defined ethnicity count", col.names = c("Officer-defined ethnicity", "Count", "Percentage")) %>% kable_styling(full_width = F)
```

```{r echo = FALSE}
self_defined_freq_data %>% kable(caption = "Self-defined ethnicity count", col.names = c("Self-defined ethnicity", "Count","Percentage")) %>% kable_styling(full_width = F)
```
...
```{r include = TRUE, echo = FALSE}
df <- data.frame("indicator" = c("Employment (2020-2021)", 
                                 "Level 4+ GCSE Maths & English (2019-2020)", 
                                 "Good level of development at age 5 (2018-2019)",
                                 "Homeless or at risk of being homeless (2020-2021)",
                                 "Police Stop and Search (2020-2021)",
                                 "Children looked after (2018-2019)"),
                 "indicator_short" = c("Employment", 
                                 "Level 4+ GCSE Maths & English", 
...
df_2$indicator_short <- fct_inorder(df_2$indicator_short) # put levels in order they appear in df
...
ggplot(df_2, aes(indicator_short, OR)) +
  ...
  geom_text(size = 3, nudge_x = -0.35, nudge_y = 0, aes(x = indicator_short,label = paste(round(original_OR, 2),"x", sep=""),hjust = 0.5)) +
...
# df_2 <- df
# 
# new_row <- as.data.frame(t(as.matrix(c("White", 1, 0, 0, 0))))
# 
# df_2[nrow(df_2)+1,] <- new_row
# 
# df_2$original_OR <- df_2$OR # keep original OR values
# df_2 <- df_2[order(df_2$OR),] # order df from lowest to highest
# df_2$indicator <- fct_inorder(df_2$indicator) # put levels in order they appear in df
# 
# df_2$y <- c(1,1,1,1,1,1,1)
# df_2$size <- df_2$OR
# df_2 <- df_2 %>%
#   mutate(
#     OR = as.numeric(OR),
...
# 
# 
# ggplot(df_2, aes(indicator, y)) +
#   geom_count(aes(size = OR)) +
#     theme_bw() +
...
A further step was taken to produce public-facing results for the dashboards that are more straightforward to interpret. This was done by calculating relative risk ratios instead of odds ratios, using the *riskratio* function from the *epitools* package. Risk ratios tend to be more intuitive to interpret for non-statisticians because the value presents relative increases or decreases in the *likelihood* of an outcome rather than in the *odds* of an outcome. Although odds ratios tend to approximate relative risk ratios when an outcome is uncommon in the comparator group (< 10%), when the outcome is common in the comparator group (> 10%) odds ratios tend to exaggerate relative risk ratios [@davies_when_1998].
...
rr_gcse <- rr_from_df(gcse_df, "Level 4+ GCSE Maths & English (2018-2019)")
rr_employment <- rr_from_df(employment_df, "Employment rate (2020-2021)")
rr_homeless <- rr_from_df(homeless_df, "Statutorily homeless or at risk (2020-2021)")
...
for(i in 1:nrow(rr_df)){
  rr_df$indicator_short[i] <- substr(rr_df$indicator[i], 1, nchar(rr_df$indicator[i]) - 12)
}

rr_df$indicator_short <- as.factor(rr_df$indicator_short)
```

This procedure yielded the statistics presented in the figure and table below.
...
rr_df_2$indicator_short <- fct_inorder(rr_df_2$indicator_short) # put levels in order they appear in df
...
ggplot(rr_df_2, aes(indicator_short, rr)) +
  ...
  geom_text(size = 3, nudge_x = -0.35, nudge_y = 0, aes(x = indicator_short,label = paste(round(original_rr, 2),"x", sep=""),hjust = 0.5)) +
...
rr_df_3 %>% kable(#"html", 
                  escape = F, 
                  row.names = F, col.names = linebreak(c("Indicator", "Relative risk", "Lower confidence interval (95%)", "Upper confidence interval (95%)")), caption = "Relative risk and confidence intervals corresponding to the above plot") %>% kable_styling(full_width = F)
...
ggplot(df_2, aes(indicator_short, OR)) +
  ...
  # geom_text(x = nrow(df) + 1.5, y = -1*reduction_factor*0.9 +.3, label = "Less likely", colour = "black", size = 3.5, hjust = 0) +
  #geom_text(size = 3, nudge_x = -0.3, nudge_y = 0, aes(x = indicator,label = paste("OR =", round(original_OR, 2)),hjust = 0)) +
  geom_text(size = 3, nudge_x = -0.3, nudge_y = 0, aes(x = indicator_short,label = paste(round(original_OR, 2),"x", sep=""),hjust = 0.5)) +
  ggtitle("The odds of outcomes for the Black population \ncompared to the White population") +
  labs(caption = "Error bars represent the 95% confidence interval")
```
...
As for Birmingham, the above results were also expressed as relative risk ratios for presentation in the public-facing dashboards. The method for doing so was exactly the same as for the Birmingham statistics above, and is provided in the code below.
...
The child population estimates drawn from the Census 2011 that supplemented the children looked after by the local authority in Birmingham are very outdated, which has the effect of reducing the accuracy of the statistics associated with this indicator. 
